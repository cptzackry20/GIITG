...
if (isset($_GET['delete_quiz'])) {
    $quizIdToDelete = $_GET['delete_quiz'];
    // Perform the database delete operation here
    $deleteQuery = "DELETE FROM quiz WHERE id = ?";
    $stmt = $conn->prepare($deleteQuery);
    $stmt->bind_param("i", $quizIdToDelete);

    if ($stmt->execute()) {
        // Quiz deleted successfully
        header("Location: managequiz.php"); // Redirect to refresh the page
        exit;
    } else {
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-7 text-right">
                <a href="addnewquiz.php" class="btn btn-success"><i class="fa fa-plus"></i> Add New Quiz</a>
            </div>
        </div>
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                if ($result && $result->num_rows > 0) {
                    // Loop through the quizzes and display them in a table
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group" role="group">
                                    <a href="editquiz.php?id=<?php echo $quizId; ?>" class="btn btn-primary mr-2">Edit</a>
                                    <a href="?delete_quiz=<?php echo $quizId; ?>" class="btn btn-danger mr-2"
                                        onclick="return confirm('Are you sure you want to delete this quiz?')">Delete</a>
                                    <a href="#" class="btn btn-info" data-toggle="modal"
                                        data-target="#quizPreviewModal" data-quizid="<?php echo $quizId; ?>">Preview</a>
                                </div>
                            </td>
                        </tr>
                        <?php
                    ...
                    // Display a message if there are no available quizzes
                    echo '<tr><td colspan="4">Sorry, there are no available quizzes right now.</td></tr>';
                }
                ?>
            </tbody>
        </table>
